...
import { CallIcon, ChevronLeftIcon, FaqIcon, BookOpenIcon, PlusIcon } from '../components/icons';

type View = 'main' | 'ticketing';

const SupportActionCard: React.FC<{ title: string; Icon: React.FC<{ className?: string }>; onClick: () => void }> = ({ title, Icon, onClick }) => (
    <button onClick={onClick} className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-lg w-full flex flex-col items-center justify-center text-center cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-32">
        <div className="w-12 h-12 bg-indigo-100 dark:bg-slate-700 rounded-full flex items-center justify-center mb-3">
            <Icon className="w-7 h-7 text-indigo-500 dark:text-indigo-400" />
        </div>
        <h2 className="text-md font-semibold text-slate-800 dark:text-slate-200">{title}</h2>
    </button>
);
...
const TicketingPage: React.FC<{onBack: () => void}> = ({ onBack }) => {
    return (
        <div className="p-4 animate-fade-in">
            ...
            <div className="mt-6">
                 <button className="w-full bg-indigo-600 text-white font-bold py-3 px-4 rounded-xl hover:bg-indigo-700 transition duration-300 flex items-center justify-center gap-2">
...
const SupportPage: React.FC = () => {
  ...
  const supportTiles = [
    { title: "سوالات متداول", Icon: FaqIcon, action: () => {} },
    { title: "آموزش و راهنما", Icon: BookOpenIcon, action: () => {} },
    { title: "ثبت درخواست (تیکت)", Icon: PlusIcon, action: () => setView('ticketing') },
    { title: "تماس با پشتیبانی", Icon: CallIcon, action: () => setShowModal(true) },
  ];
    ...
    <div className="p-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        {supportTiles.map((tile, index) => (
          <SupportActionCard key={index} {...tile} />
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6 max-w-sm w-full">
            ...
            <p className="mb-6 text-slate-600 dark:text-slate-300 text-center">
              پذیرنده گرامی، جهت جلوگیری از اتلاف وقت، لطفاً از اینکه سؤال مورد نظر شما در قسمت سوالات متداول نمی‌باشد اطمینان حاصل کنید.
            </p>
            <div className="space-y-3">
                <a href="tel:+982184080" className="w-full flex items-center justify-center gap-2 border border-indigo-600 text-indigo-600 font-bold py-2 px-4 rounded-xl hover:bg-indigo-50 dark:hover:bg-slate-700 transition-colors">
                    <CallIcon />
                    <span dir="ltr">+98 21 840 80</span>
                </a>
                <button 
